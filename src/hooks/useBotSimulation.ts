...
function pickBotAction(state: GameState): { name: string; command: string } | null {
  ...
  for (const [id, station] of Object.entries(state.stations)) {
    if (id !== 'cutting_board' && id !== 'mixing_bowl' && !station.overheated && station.heat >= 60) return { name, command: `cool ${id}` }
  }
  ...
  for (const order of state.orders) {
    ...
    for (const step of recipe.steps) {
      ...
      const capacity = (step.station === 'cutting_board' || step.station === 'mixing_bowl')
